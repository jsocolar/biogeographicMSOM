# Comparing runtimes across varying numbers of threads.
#
# Simulating under:
...
# - reduce_sum has been run on 1 thread, with 4 chains 
#   ("check_reduceSum_1thread.R")and produces correct result
#
# Running here 1 chain, varying number of threads (convergence of chains having 
# been checked on 1 thread:4chain run). Note, data across all checks simulated 
# under seed=101, so samples are comparable across all checks (e.g. 'ragged', 
# 'original').
...
# Run 1 thread ----
# note: calling the same model each time, just with varying number of threads. 
# do I need to recall the model after declaring threads, or would just changing
...
rm(list = c("samps_1", "samps_2", "samps_4", "samps_R"))
save.image("workspace/compare_parallel_run_times.RData")
# Run 8 thread ----
stan_data$grainsize <- 2880/8
set_num_threads(8)
mod_P <- cmdstan_model("stan_files/occupancyMod_ragged_parallel.stan",
                       threads=TRUE)
time_8 <- system.time(samps_8 <- mod_P$sample(data = stan_data,
                                              num_chains = num_chains,
                                              num_cores = num_chains))

# Run 16 thread ----
# stan_data$grainsize <- 2880/16
# set_num_threads(16)
# mod_P <- cmdstan_model("stan_files/occupancyMod_ragged_parallel.stan",
#                        threads=TRUE)
# time_16 <- system.time(samps_16 <- mod_P$sample(data = stan_data,
#                                                 num_chains = num_chains,
#                                                 num_cores = num_chains))
...
sub_8 <- samps_8$draws() %>%
    subset_draws(., variable = c("mu_b0", "sigma_b0", "mu_d0", "sigma_d0"))
# sub_16 <- samps_16$draws() %>%
#     subset_draws(., variable = c("mu_b0", "sigma_b0", "mu_d0", "sigma_d0"))

# Save outputs ----
# Reduce filesize by dropping full set of samples
rm(list = c("samps_8"))
save.image("workspace/compare_parallel_run_times.RData")
