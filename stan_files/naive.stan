...
model {
  // Transformed parameters that I don't want to monitor
    ...
    vector[n_species] logit_psi = mu_b0 + b0[id_sp] + b1_elev[id_sp] .* elev;  // logit occupancy probability
...
generated quantities {
  ...
  for (n in 1:nq) {
    log_lik[n] = bernoulli_logit_lpmf(1 | mu_b0 + b0[id_sp[n]] + b1_elev[id_sp[n]] * elev[n]) + binomial_logit_lpmf(det_data[n] | 5, d0[id_sp[n]]);
  }
  for (n in (nq+1):n_tot) {
    log_lik[n] = log_sum_exp(log_inv_logit(mu_b0 + b0[id_sp[n]] + b1_elev[id_sp[n]] * elev[n]) + log1m_inv_logit(d0[id_sp[n]]) * n_visit, log1m_inv_logit(mu_b0 + b0[id_sp[n]] + b1_elev[id_sp[n]] * elev[n]));
  }
}
